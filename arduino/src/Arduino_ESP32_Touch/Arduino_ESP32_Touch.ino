/********************
  - Executive Order Corporation - Arduino Tron ESP32 MQTT Telemetry Transport Machine-to-Machine(M2M)/Internet of Things(IoT)
...
/********************
  This is an example howto use Touch Intrrerupts on ESP32
...
void setup(void) {
  pinMode(LED_BUILTIN, OUTPUT); // Declaring Arduino LED pin as output
//pinMode(LED1, OUTPUT);
//pinMode(LED2, OUTPUT);
//pinMode(LED3, OUTPUT);
//pinMode(LED4, OUTPUT);
...
//pinMode(BUTTON8, INPUT);
  ...
  delay(500); // Give time to bring up serial monitor

  // Connect to WiFi network
  Serial.println("Executive Order Corporation - Arduino Tron ESP32 MQTT Telemetry Transport Machine-to-Machine(M2M)/Internet of Things(IoT)");
...
//  }
  ...
  touchAttachInterrupt(T5, gotTouch5, touchThreshold);
  touchAttachInterrupt(T6, gotTouch6, touchThreshold);
  touchAttachInterrupt(T7, gotTouch7, touchThreshold);
  delay(200);
}
...
void arduinoTronSend()
{
  ...
  Serial.print("Executive Order Corporation - Arduino Tron ESP32 MQTT Telemetry Transport Machine-to-Machine(M2M)/Internet of Things(IoT) ");
